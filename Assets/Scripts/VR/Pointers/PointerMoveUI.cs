...
public class PointerMoveUI : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler {
    ...
    {
        if (grabbedUI) {
            if (left && ViveInput.GetPressUpEx(HandRole.LeftHand, ControllerButton.Grip)) {
                releaseMenu();
            }
            if (!left && ViveInput.GetPressUpEx(HandRole.RightHand, ControllerButton.Grip)) {
                releaseMenu();
            }

            // Joystick Y → acercar/alejar el panel a lo largo del rayo del mando
            HandRole hand = left ? HandRole.LeftHand : HandRole.RightHand;
            float joyY = ViveInput.GetAxisEx(hand, ControllerAxis.JoystickY);
            if (Mathf.Abs(joyY) > 0.1f) {
                Transform target = moveParent ? transform.parent : transform;
                GameObject ctrl = left ? UnityMolMain.getLeftController() : UnityMolMain.getRightController();
                Vector3 dir = ctrl != null ? ctrl.transform.forward : Vector3.forward;
                target.position += dir * joyY * depthScrollSpeed * Time.deltaTime;
            }
        }
        else {
            if (ViveInput.GetPressDownEx(HandRole.RightHand, ControllerButton.Grip) && isPointerEntered)
            {
                left = false;
                if (moveParent)
            ...
            if (!grabbedUI && ViveInput.GetPressDownEx(HandRole.LeftHand, ControllerButton.Grip) && isPointerEntered)
            {
                left = true;
                if (moveParent)
                    grabMenu(gameObject.transform.parent.gameObject);
    ...
    private void grabMenu(GameObject toGrab) {
        ...
        if (left && cgsL != null && cgsL.grabbedMolecule != null)
            return;
        if (!left && cgsR != null && cgsR.grabbedMolecule != null)
            return;

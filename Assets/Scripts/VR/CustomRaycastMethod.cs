...
public class CustomRaycastMethod : BaseRaycastMethod
{
    ...
    public override void Raycast(Ray ray, float distance, List<RaycastResult> raycastResults)
    {
        ...
        bool padOrAKey =
            ...
            ViveInput.GetPress(HandRole.LeftHand,  ControllerButton.PadTouch)  ||
            ViveInput.GetPress(HandRole.RightHand, ControllerButton.AKeyTouch) ||
            ViveInput.GetPress(HandRole.LeftHand,  ControllerButton.AKeyTouch);
        if (!padOrAKey) {
            return;
        }

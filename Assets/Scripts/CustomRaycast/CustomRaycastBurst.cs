...
public class CustomRaycastBurst {
	...
	public UnityMolAtom customRaycastAtomBurst(Vector3 origin, Vector3 direction,
		...
		// Elegir el átomo más frontal usando tc (proyección sobre el rayo),
		// no squaredDist — un átomo de atrás puede estar más cerca en 3D si
		// el rayo pasa por su centro, lo que causaba selecciones incorrectas.
		float3 fOri = (float3)origin;
		float3 fDir = (float3)direction;

		float minDist = float.MaxValue;
		int idMin = -1;
		for (int i = 0; i < results.Length; i++) {
			if (results[i]) {
				float tc = math.dot(atomPos[i] - fOri, fDir);
				if (tc < minDist) {
					minDist = tc;
					idMin = i;
				}
		...
		float minDistEx = float.MaxValue;
		int idMinEx = -1;
			...
			if (atomPosTrajExtract.IsCreated && atomPosTrajExtract.Length != 0) {
				...
				for (int i = 0; i < resultsTrajExtract.Length; i++) {
					if (resultsTrajExtract[i]) {
						float tc = math.dot(atomPosTrajExtract[i] - fOri, fDir);
						if (tc < minDistEx) {
							minDistEx = tc;
							idMinEx = i;
						}

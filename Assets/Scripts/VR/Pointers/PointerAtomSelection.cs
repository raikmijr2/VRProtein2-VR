...
public class PointerAtomSelection : MonoBehaviour {
    ...
    HashSet<UnityMolAtom> highlightedAtoms = new HashSet<UnityMolAtom>();

    bool isPressed = false;
    bool longPress = false;
    UnityMolAtom hoveredAtom = null;
    float longPressLimit = 2.0f;
    ...
    void padClickedAdditive() { doSelection(); }

    void doSelection() {

        // Usar el átomo que el hover detectó continuamente — es más fiable
        ...
        if (a == null) {
            ...
            if (hoverScript != null)
                hoverScript.GetPointerRay(out rayOrig, out rayDir);
            else {
                RigidPose cpose2 = VivePose.GetPose(curRole);
                rayOrig = cpose2.pos;
                rayDir  = cpose2.forward;
            }
        ...
        if (selM.selectionMode == UnityMolSelectionManager.SelectionMode.Atom) {
            ...
            hoveredSelection = hoveredAtom.residue.ToSelection(false);
        }
        else if (selM.selectionMode == UnityMolSelectionManager.SelectionMode.Chain) {
            hoveredSelection = hoveredAtom.residue.chain.ToSelection(false);
        }
    ...
    void ApplyHighlight(UnityMolSelection sel) {
        ClearHighlight();
        AddHighlight(sel);
    }
    ...
    void ClearHighlight() {
        if (highlightedAtoms.Count == 0) return;
        var repManager = UnityMolMain.getRepresentationManager();
        if (repManager == null) { highlightedAtoms.Clear(); return; }

        foreach (var a in highlightedAtoms) {
            foreach (var rep in repManager.representations) {
                if (rep.selection != null && rep.selection.atomToIdInSel.ContainsKey(a))
                    rep.ResetColor(a);
            }
    ...
    void buttonReleased() {
        ...
        if (longPress) {
            ...
            // BUG FIX: a diferencia de doSelection() (clic normal), esta rama nunca llamaba a
            // AddHighlight() — la selección por esfera actualizaba los datos de selección pero
            // nunca coloreaba de amarillo los átomos seleccionados.
            AddHighlight(newSel);
